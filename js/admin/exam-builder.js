...
export async function openAssessmentBuilder(assessmentId = null) {
    ...
    <div class="modal-content" style="max-width: 960px; height: 90vh; display: flex; flex-direction: column;">
      <div class="modal-header d-flex justify-between align-center p-3" style="border-bottom: 1px solid var(--clr-border);">
        <div>
          ...
          <p class="text-muted m-0" style="font-size: 0.8rem;">Create Evaluations or Comprehensive Exams with frozen question snapshots</p>
        </div>
      ...
      <div class="modal-body p-0" style="flex: 1; overflow: hidden; display: flex; flex-direction: column;">
        ...
        <div class="builder-content" style="flex: 1; overflow-y: auto; padding: 24px; background: var(--clr-bg-1);">
              ...
              <div class="form-grid">
                <div class="form-group form-group-full mb-3">
                  ...
                  <div class="d-flex gap-4">
                    ...
                    <label style="cursor:pointer;display:flex;align-items:center;gap:8px;">
                      <input type="radio" name="wiz-type" value="EXAM" />
                      <strong>Comprehensive Exam</strong> <span class="text-muted text-xs">(Derived from Evaluation sequences)</span>
                    </label>
                  </div>
                </div>
          ...
          <div id="tab-topics" class="b-tab-content hidden">
            ...
            <!-- Container for EXAM: sequence of evaluations derivation -->
            <div id="wiz-exam-sequence-section" class="card p-4 mb-4 hidden" style="background:rgba(255,255,255,0.02);border:1px solid var(--clr-border);">
              <div class="d-flex justify-between align-center flex-wrap gap-2 mb-3">
                <div>
                ...
                <div>
                  <span class="badge badge-primary p-2" style="font-size:0.85rem;">
                    Derived Questions: <strong id="wiz-exam-q-count">0</strong>
                  </span>
                </div>
              </div>
  ...
  const close = () => {
    builderDiv.remove();
    if (window.loadSection) window.loadSection('exams');
  };
  ...
  try {
    ...
    const subSel = document.getElementById('wiz-subject');
    subSel.innerHTML = allSubjects.map(s => `<option value="${s.id}">${escapeHtml(s.name)}</option>`).join('');
    ...
    // Toggle Evaluation vs Exam UI in Tab 2
    ...
    const examSection = document.getElementById('wiz-exam-sequence-section');

    const updateTypeUI = () => {
      const isExam = builderDiv.querySelector('input[name="wiz-type"]:checked').value === 'EXAM';
      evalSection.classList.toggle('hidden', isExam);
      examSection.classList.toggle('hidden', !isExam);
      if (isExam) {
        renderExamEvaluations();
      } else {
      ...
      if (!filteredTopics.length) {
        container.innerHTML = `
          <div class="p-4 text-center rounded" style="background:rgba(255,255,255,0.02);border:1px dashed var(--clr-border);">
            <p class="text-muted text-xs mb-3">No topics found for this subject yet. You can add one quickly below:</p>
    ...
    const renderExamEvaluations = () => {
      ...
      if (!evals.length) {
        container.innerHTML = `<p class="text-muted text-xs p-3 text-center">No published evaluations found for this subject. Create Evaluations first.</p>`;
        updateExamDerivedCoverage();
        return;
      }
    ...
    const updateExamDerivedCoverage = async () => {
      ...
      if (!derivedTopics.length) {
        ...
        document.getElementById('wiz-exam-q-count').textContent = '0';
        return;
      }
      ...
      document.getElementById('wiz-exam-q-count').textContent = qCount;
    };
    ...
    if (assessmentId) {
      ...
      if (!editAssessment) {
        try {
          ...
          editAssessment = (allExams || []).find(e => e.id === assessmentId);
        } catch (_) {}
      }
    }
    ...
    if (editAssessment) {
      ...
      if (prereqSelEl && (editAssessment.prerequisite_assessment_id || editAssessment.prerequisite_exam_id)) {
        prereqSelEl.value = editAssessment.prerequisite_assessment_id || editAssessment.prerequisite_exam_id;
      }
    ...
    document.getElementById('btn-assign-next').onclick = () => {
      ...
      const qTotal = (typeVal === 'EXAM')
        ? document.getElementById('wiz-exam-q-count').textContent
