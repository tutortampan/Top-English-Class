...
DROP TABLE IF EXISTS levels CASCADE; -- Drop legacy levels
DROP TABLE IF EXISTS exam_classes CASCADE; -- Drop legacy
DROP TABLE IF EXISTS class_subjects CASCADE; -- Drop legacy
DROP TABLE IF EXISTS classes CASCADE; -- Drop legacy

-- ============================================================
-- 1. CORE TABLES
-- ============================================================

-- INSTITUTIONS (Formerly Programs)
...
-- PROGRAM_SUBJECTS (Many-to-Many: Program <-> Subject)
...
-- BATCHES (Hierarchy: Institution -> Program -> Batch)
...
ALTER TABLE exams ADD COLUMN IF NOT EXISTS program_id UUID REFERENCES programs(id) ON DELETE SET NULL;
...
-- EXAM_PROGRAMS (Many-to-Many: Exam <-> Program)
...
CREATE POLICY "service_role_all_program_subjects" ON program_subjects FOR ALL TO service_role USING (TRUE) WITH CHECK (TRUE);
...
CREATE POLICY "service_role_all_exam_programs" ON exam_programs FOR ALL TO service_role USING (TRUE) WITH CHECK (TRUE);
...
CREATE POLICY "public_read_institutions" ON institutions FOR SELECT TO anon USING (deleted_at IS NULL AND is_active = TRUE);
...
CREATE POLICY "public_read_students_by_program" ON students FOR SELECT TO anon USING (is_active = TRUE AND deleted_at IS NULL);
...
CREATE POLICY "admin_all_program_subjects" ON program_subjects FOR ALL TO anon USING (TRUE) WITH CHECK (TRUE);
...
CREATE POLICY "admin_all_exam_programs" ON exam_programs FOR ALL TO anon USING (TRUE) WITH CHECK (TRUE);
...
VALUES ('11111111-1111-1111-1111-111111111111', 'General English Institution', TRUE);

-- Permanent Default Institutions, Programs & Subjects:
-- 1. CEC with Camp Program & Vocabularies Subject
INSERT INTO institutions (id, name, is_active)
VALUES ('24d52c09-f2e6-4bbe-b8ab-3eaa41c8b333', 'CEC', TRUE)
ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, deleted_at = NULL;

INSERT INTO programs (id, institution_id, name, is_active)
VALUES ('d4c6d85a-44b7-4121-acae-28f7b9e70dcd', '24d52c09-f2e6-4bbe-b8ab-3eaa41c8b333', 'Camp', TRUE)
ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, deleted_at = NULL;

INSERT INTO subjects (id, institution_id, name, is_active)
VALUES ('99316504-99dd-444c-88bd-6ce073f878d3', '24d52c09-f2e6-4bbe-b8ab-3eaa41c8b333', 'Vocabularies', TRUE)
ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, deleted_at = NULL;

-- 2. Sheraton with Morning & Afternoon Programs & Vocabularies Subject
INSERT INTO institutions (id, name, is_active)
...
-- 3. Tamata with Hospitality Program & Vocabularies Subject
INSERT INTO institutions (id, name, is_active)
...
-- Demo Program
INSERT INTO programs (id, institution_id, name, is_active)
VALUES ('22222222-2222-2222-2222-222222222222', '11111111-1111-1111-1111-111111111111', 'Program A', TRUE);

-- Demo Batch
INSERT INTO batches (id, program_id, name, is_active)
...
INSERT INTO students (id, institution_id, program_id, batch_id, name, gender, birth_date, pin_hash, is_active)
VALUES (
