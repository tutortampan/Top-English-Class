...
## 2. Core Hierarchy
- INSTITUTION -> PROGRAM -> BATCH (Contains Students)
- INSTITUTION -> PROGRAM -> CLASS (Contains Assessments)
- CLASS -> TOPIC -> ASSESSMENT -> MODULE
- Assessment Assignment -> Assigned to Batch OR Assigned to Student directly.
- Student + Class -> Progress
- Student + Assessment -> Submission (Evaluated via AI Pipeline)

## 3. Key Non-Negotiable Requirements
- **Admin Navigation**: Exactly four primary tabs (`DATABASE`, `PROGRAM`, `STUDENT`, `ASSESSMENT`).
...
- **Historical Snapshot**: `assessment_results` logs every attempt payload and AI evaluation in JSON.
